...
class Nnetmon(object):
    def __init__(self, net, batches_per_epoch=12, rec_interval=10, decay_rate=0.99, replacement_rate=1e-4,
        ...
        self.sid = 0
        self.epoch, self.e_rank, self.dead_n, self.f_val, self.mag_w = [], [], [], [], []
        ...
        with torch.no_grad():
            ...
            if isinstance(current_layer, Linear):
                ...
                if isinstance(next_layer, Conv2d):
                    ...
                    self.mean_feature_act[layer_idx] += (1 - self.decay_rate) * features.mean(dim=0). \
                        view(-1, self.num_last_filter_outputs).mean(dim=1)
                    self.mean_abs_feature_act[layer_idx] += (1 - self.decay_rate) * features.abs().mean(dim=0). \
                        view(-1, self.num_last_filter_outputs).mean(dim=1)

            if isinstance(current_layer, Linear):
                self.val_p[layer_idx] += (1 - self.decay_rate) * features.abs().mean(dim=0) * output_wight_mag
                self.g_rate[layer_idx] += (1 - self.decay_rate) * (features - self.h_t_1[layer_idx]).abs().mean(dim=0)
                self.mag[layer_idx] += (1 - self.decay_rate) * output_wight_mag / input_wight_mag

            elif isinstance(current_layer, Conv2d):
                if isinstance(next_layer, Conv2d):
                    self.val_p[layer_idx] += (1 - self.decay_rate) * features.abs().mean(
                        dim=(0, 2, 3)) * output_wight_mag
                    self.g_rate[layer_idx] += (1 - self.decay_rate) * (features - self.h_t_1[layer_idx]).abs().mean(
                        dim=(0, 2, 3))
                    self.mag[layer_idx] += (1 - self.decay_rate) * output_wight_mag / input_wight_mag
                else:
                    self.val_p[layer_idx] += (1 - self.decay_rate) * output_wight_mag.view(-1,
                                                                                           self.num_last_filter_outputs). \
                        mean(dim=1) * features.abs().mean(dim=0).view(-1, self.num_last_filter_outputs).mean(dim=1)
                    self.g_rate[layer_idx] += (1 - self.decay_rate) * (features - self.h_t_1[layer_idx]).abs().mean(
                        dim=0). \
                        view(-1, self.num_last_filter_outputs).mean(dim=1)
                    self.mag[layer_idx] += (1 - self.decay_rate) * output_wight_mag.view(-1,
                                                                                         self.num_last_filter_outputs). \
                        mean(dim=1) / input_wight_mag
            ...
            elif self.util_type in ['nova_contribution', 'nova_contribution dead neuron'] or (self.util_type is None):
