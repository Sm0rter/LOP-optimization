...
def main(arguments):
    ...
    parser.add_argument('-c', help="Path of the file containing the parameters of the experiment",
                        type=str, default='../lop/permuted_mnist/cfg/bp/std_net.json')
    ...
    print(list_params, hyper_param_settings)

    # make a directory for temp cfg files
#    bash_command = "mkdir -p temp_cfg/"
#    subprocess.Popen(bash_command.split(), stdout=subprocess.PIPE)
    os.makedirs('temp_cfg_mnist', exist_ok=True)
    print('made temp_cfg_mnist')
...
#        subprocess.Popen(bash_command.split(), stdout=subprocess.PIPE)
        ...
        for idx in tqdm(range(params['num_runs'])):
            ...
            """
            new_cfg_file = 'temp_cfg_mnist/'+str(setting_index*params['num_runs']+idx)+'.json'
