...
def online_expr(params: {}):
    ...
    if agent_type == 'linear':
        print('initiating linear network')
        net = MyLinear(
    ...
    if agent_type in ['bp', 'linear', "l2"]:
        ...
        print('initiating continual backprop')
    ...
    iter = 0
    with open('../lop/permuted_mnist/data/mnist_', 'rb+') as f:
    ...
    for task_idx in (range(num_tasks)):
        ...
        if agent_type != 'linear':
            with torch.no_grad():
                ...
                for rep_layer_idx in range(num_hidden_layers):
                    print('CALCULATING ERANK AND DEADNEURONS')
        ...
        if task_idx % save_after_every_n_tasks == 0:
            ...
            print('files saved in', params['data_file'])
...
def main(arguments):
    ...
    parser.add_argument('-c', help="Path to the file containing the parameters for the experiment",
                        type=str, default='temp_cfg_mnist/0.json')
